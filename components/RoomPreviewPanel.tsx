  ...
  <>
    <section className={`order-4 rounded-[32px] border p-5 shadow-[0_20px_80px_rgba(84,102,140,0.14)] backdrop-blur-2xl lg:order-3 ${
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className={`text-[11px] uppercase tracking-[0.24em] ${theme === 'dark' ? 'text-stone-400' : 'text-stone-500'}`}>{labels.roomPreview}</p>
          <h2 className={`mt-2 font-display text-3xl ${theme === 'dark' ? 'text-stone-100' : 'text-stone-900'}`}>{labels.openPreview}</h2>
    ...
    {isOpen && (
      <div className="fixed inset-0 z-[90] flex items-end justify-center bg-black/50 p-0 sm:items-center sm:p-6">
        <div className={`max-h-[92vh] w-full overflow-auto rounded-t-[28px] border p-4 shadow-[0_30px_80px_rgba(0,0,0,0.28)] sm:max-w-3xl sm:rounded-[30px] sm:p-5 ${
          theme === 'dark'
          ...
          <div className="mb-4 flex items-start justify-between gap-4">
            <div>
              <p className={`text-[11px] uppercase tracking-[0.24em] ${theme === 'dark' ? 'text-stone-400' : 'text-stone-500'}`}>{labels.roomPreview}</p>
              <h2 className={`mt-2 font-display text-3xl ${theme === 'dark' ? 'text-stone-100' : 'text-stone-900'}`}>{labels.aiPreviewDemo}</h2>
            </div>
            <button
              type="button"
              onClick={onClose}
          ...
          <div className={`overflow-hidden rounded-[26px] border p-3 ${theme === 'dark' ? 'border-white/10 bg-[linear-gradient(160deg,_rgba(42,37,33,0.95),_rgba(27,24,21,0.92))]' : 'border-stone-900/6 bg-[linear-gradient(160deg,_rgba(243,238,231,0.95),_rgba(232,224,214,0.92))]'}`}>
            <div className="relative aspect-[4/5] overflow-hidden rounded-[22px] bg-[url('data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22320%22 height=%22440%22 viewBox=%220 0 320 440%22%3E%3Cdefs%3E%3ClinearGradient id=%22g%22 x1=%220%22 x2=%221%22 y1=%220%22 y2=%221%22%3E%3Cstop stop-color=%22%23f6efe5%22/%3E%3Cstop offset=%221%22 stop-color=%22%23e0d4c4%22/%3E%3C/linearGradient%3E%3C/defs%3E%3Crect width=%22320%22 height=%22440%22 fill=%22url(%23g)%22/%3E%3Crect x=%2234%22 y=%2248%22 width=%22252%22 height=%22126%22 rx=%2222%22 fill=%22%23ffffff%22 fill-opacity=%220.45%22/%3E%3Crect x=%2256%22 y=%22238%22 width=%22212%22 height=%2290%22 rx=%2216%22 fill=%22%23d8c1a3%22 fill-opacity=%220.42%22/%3E%3Crect x=%2280%22 y=%22334%22 width=%22158%22 height=%2248%22 rx=%2214%22 fill=%22%23ece3d7%22 fill-opacity=%220.72%22/%3E%3C/svg%3E')] bg-cover bg-center">
              ...
              <div
                className={`absolute left-1/2 top-[62%] -translate-x-1/2 rounded-[18px] border border-white/40 bg-white/15 p-2 shadow-[0_16px_30px_rgba(40,27,15,0.18)] backdrop-blur-sm transition-all ${
                  roomPlacementMode === 'center'
                    ? 'w-[46%] max-w-[210px]'
                    : 'bottom-4 left-4 right-4 top-auto w-auto translate-x-0'
                } ${demoPreviewApplied ? 'opacity-100' : 'opacity-55'}`}
              >
                <img
                  src={product.images[0]}
                  alt={`${product.name} room preview`}
                  className={`w-full rounded-[16px] object-cover ${
                    roomPlacementMode === 'center' ? 'aspect-[4/3]' : 'aspect-[16/7]'
                  }`}
                />
              </div>
            </div>
          ...
          <div className="mt-4">
            <label className={`flex cursor-pointer items-center justify-center rounded-[22px] border border-dashed px-4 py-4 text-sm font-semibold transition ${
              theme === 'dark'
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            {(['center', 'coverage'] as RoomPlacementMode[]).map((mode) => {
              const active = roomPlacementMode === mode;
              const label = mode === 'center' ? labels.centerPlacement : labels.fullCoverage;
              const hint = mode === 'center' ? labels.centerPlacementHint : labels.fullCoverageHint;
              ...
              return (
                ...
                >
                  ...
                  <div className={`mt-1 text-xs ${active ? (theme === 'dark' ? 'text-stone-800/70' : 'text-white/75') : theme === 'dark' ? 'text-stone-400' : 'text-stone-500'}`}>
                    {hint}
                  </div>
                </button>
              );
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            <label className={`rounded-[22px] border px-4 py-3 ${theme === 'dark' ? 'border-white/10 bg-white/5' : 'border-stone-200 bg-white/80'}`}>
              <div className={`text-[11px] uppercase tracking-[0.22em] ${theme === 'dark' ? 'text-stone-400' : 'text-stone-500'}`}>{labels.roomWidth}</div>
              <input
                value={roomDimensions.width}
                onChange={(event) => onDimensionsChange({ ...roomDimensions, width: event.target.value })}
                className={`mt-2 w-full bg-transparent text-sm outline-none ${theme === 'dark' ? 'text-stone-100 placeholder:text-stone-500' : 'text-stone-700 placeholder:text-stone-400'}`}
                placeholder="4.0 m"
              />
            </label>
            ...
            <label className={`rounded-[22px] border px-4 py-3 ${theme === 'dark' ? 'border-white/10 bg-white/5' : 'border-stone-200 bg-white/80'}`}>
              ...
              <input
                ...
                onChange={(event) => onDimensionsChange({ ...roomDimensions, height: event.target.value })}
                className={`mt-2 w-full bg-transparent text-sm outline-none ${theme === 'dark' ? 'text-stone-100 placeholder:text-stone-500' : 'text-stone-700 placeholder:text-stone-400'}`}
                placeholder="5.5 m"
              />
            </label>
          ...
          <button
            type="button"
            onClick={onApply}
            className={`mt-4 w-full rounded-[22px] border px-5 py-4 text-sm font-semibold transition ${
              theme === 'dark'
